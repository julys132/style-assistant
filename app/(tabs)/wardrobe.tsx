...
type WardrobeGridSize = "tiny" | "small" | "medium" | "large";
...
const COLOR_NAME_PREFIX: Record<string, string> = {
  Black: "black",
  White: "white",
  Navy: "navy",
  Gray: "gray",
...
const LEADING_COLOR_WORDS = new Set(
  Object.values(COLOR_NAME_PREFIX).flatMap((value) => value.split(/\s+/)).concat([
    "grey",
    "charcoal",
    "heather",
  ]),
);
const LEADING_COLOR_PHRASES = new Set(["dark heather", "light heather", "off white"]);
const GENERIC_ITEM_WORDS = new Set(
  Object.values(CATEGORY_FALLBACK_NAME).flatMap((value) => value.split(/\s+/)),
);
...
function buildFallbackItemName(category: string, color: string): string {
  const noun = CATEGORY_FALLBACK_NAME[category] || "item";
  const colorPrefix = COLOR_NAME_PREFIX[color] || color.trim().toLowerCase();
  return colorPrefix ? `${colorPrefix} ${noun}`.trim() : noun;
}

function syncAutoGeneratedNameWithColor(name: string, nextColor: string, category: string): string {
  const trimmed = name.trim();
  const colorPrefix = COLOR_NAME_PREFIX[nextColor] || nextColor.trim().toLowerCase();
  if (!trimmed) {
    return buildFallbackItemName(category, nextColor);
  }

  const tokens = trimmed.split(/\s+/);
  const lowerTokens = tokens.map((token) => token.toLowerCase());
  const firstTwoTokens = `${lowerTokens[0] || ""} ${lowerTokens[1] || ""}`.trim();

  if (LEADING_COLOR_PHRASES.has(firstTwoTokens)) {
    return [colorPrefix, ...tokens.slice(2)].join(" ").trim();
  }

  if (LEADING_COLOR_WORDS.has(lowerTokens[0] || "")) {
    tokens[0] = colorPrefix;
    return tokens.join(" ").trim();
  }

  if (GENERIC_ITEM_WORDS.has(lowerTokens[0] || "") || isGenericSuggestedName(trimmed)) {
    return `${colorPrefix} ${trimmed}`.trim();
  }
...
function ClothingCard({
  ...
  return (
    <Animated.View
      ...
      style={[styles.clothingCard, { width: cardWidth }, compact && styles.clothingCardCompact]}
    >
      {displayImageUri ? (
        <Image
          source={{ uri: displayImageUri }}
          style={[styles.clothingImage, compact && styles.clothingImageCompact]}
          contentFit="cover"
        />
      ) : (
        <View style={[styles.clothingImagePlaceholder, compact && styles.clothingImageCompact]}>
          <MaterialCommunityIcons name="hanger" size={32} color={Colors.textMuted} />
        </View>
      )}
      <View style={[styles.clothingInfo, compact && styles.clothingInfoCompact]}>
        <Text style={[styles.clothingName, compact && styles.clothingNameCompact]} numberOfLines={1}>{item.name}</Text>
        <Text style={[styles.clothingMeta, compact && styles.clothingMetaCompact]}>
...
export default function WardrobeScreen() {
  ...
  const gridGap = gridSize === "tiny" ? 8 : 10;
  const gridColumns =
    gridSize === "tiny"
      ? screenWidth >= 1400
        ? 7
  ...
  const handleSuggestDetails = useCallback(async () => {
    ...
    setSuggesting(true);
    try {
      const primaryModel: SuggestModel = SHOW_DEVELOPER_MODEL_SWITCH ? suggestModel : DEFAULT_SUGGEST_MODEL;
      const fallbackModel: SuggestModel = primaryModel === "llava" ? "uform" : "llava";

      const requestSuggestion = async (model: SuggestModel) =>
        apiClient.suggestWardrobeDetails({
          imageBase64,
          mimeType: imageMimeType,
          model,
        });

      const normalizeSuggestion = (suggestion: Awaited<ReturnType<typeof requestSuggestion>>) => {
        ...
        return {
          ...
          hasData: Boolean(
            (normalizedName && !isGenericSuggestedName(normalizedName)) ||
              normalizedCategory ||
              normalizedColor ||
              normalizedPattern ||
              normalizedShade,
      ...
      let suggestion = await requestSuggestion(primaryModel);
      let normalized = normalizeSuggestion(suggestion);

      if (!normalized.hasData && fallbackModel !== primaryModel) {
        try {
          const fallbackSuggestion = await requestSuggestion(fallbackModel);
          const normalizedFallback = normalizeSuggestion(fallbackSuggestion);
          if (normalizedFallback.hasData) {
            suggestion = fallbackSuggestion;
            normalized = normalizedFallback;
          }
        } catch {
          // Keep primary result; user still gets any available data.
        }
      }

      console.log("[wardrobe suggest] raw suggestion:", suggestion);

      const resolvedCategory = normalized.category || category;
      ...
      if (normalized.name && !isGenericSuggestedName(normalized.name)) {
        setName(normalized.name);
        setIsNameAutoGenerated(true);
      } else if (resolvedCategory || resolvedColor) {
        const fallbackName = buildFallbackItemName(resolvedCategory, resolvedColor);
        if (fallbackName) {
      ...
      if (normalized.shade) {
        setSuggestedShade(normalized.shade);
        setShade(normalized.shade);
      } else {
        setSuggestedShade("");
      }
  ...
  }, [imageBase64, imageMimeType, suggestModel, category, color]);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ClothingCard
            ...
            cardWidth={cardWidth}
            compact={gridSize === "tiny"}
          />
        )}
        numColumns={gridColumns}
        key={`wardrobe-grid-${gridSize}-${gridColumns}`}
        columnWrapperStyle={gridColumns > 1 ? [styles.row, gridSize === "tiny" && styles.rowCompact] : undefined}
                        ...
                        <Pressable
                          ...
                          onPress={() => {
                            ...
                            if (isNameAutoGenerated) {
                              setName((currentName) =>
                                syncAutoGeneratedNameWithColor(currentName, c, category),
                              );
                            }
                            Haptics.selectionAsync();
                          }}
...
const styles = StyleSheet.create({
  ...
  rowCompact: { gap: 8, marginBottom: 8 },
  emptyContainer: {
    flex: 1,
    justifyContent: "center",
    alignItems: "center",
  ...
  clothingInfoCompact: { padding: 8, gap: 1 },
